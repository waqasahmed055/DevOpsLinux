...
echo ""
echo "Adding more NORMAL commands (these should be PRESERVED):"
echo "======================================================="

# More normal commands
add_to_history "vim /etc/hosts"
add_to_history "nano ~/.bashrc"
add_to_history "less /var/log/syslog"
add_to_history "head -20 /etc/passwd"
add_to_history "tail -100 /var/log/auth.log"
add_to_history "wc -l /etc/passwd"
add_to_history "sort /etc/passwd"
add_to_history "uniq -c access.log"
add_to_history "cut -d: -f1 /etc/passwd"
add_to_history "tr '[:lower:]' '[:upper:]' < file.txt"
add_to_history "xargs rm < filelist.txt"
add_to_history "tee output.txt < input.txt"
add_to_history "split -l 1000 largefile.txt"
add_to_history "join file1.txt file2.txt"
add_to_history "paste file1.txt file2.txt"
add_to_history "diff file1.txt file2.txt"
add_to_history "comm -12 sorted1.txt sorted2.txt"
add_to_history "ln -s /path/to/file symlink"
add_to_history "readlink -f symlink"
add_to_history "stat filename"

# Force write history to disk
history -w

echo ""
echo "=== TESTING SCRIPT COMPLETED ==="
echo ""
echo "Current history file status:"
echo "Total lines in ~/.bash_history: $(wc -l < ~/.bash_history)"
echo ""
echo "Lines containing password keywords:"
grep -i -c -E "(password|passwd|pwd|pass=|--password|-p |secret|token|key=)" ~/.bash_history || echo "0"
echo ""
echo "Lines NOT containing password keywords:"
grep -v -i -c -E "(password|passwd|pwd|pass=|--password|-p |secret|token|key=)" ~/.bash_history || echo "0"
echo ""
echo "Now run your Ansible playbook to test the password removal functionality!"
echo ""
echo "After running the playbook, check:"
echo "1. Total lines should be reduced"
echo "2. Lines with password keywords should be 0"
echo "3. Normal commands should still be present"
echo ""
echo "Quick verification commands:"
echo "- wc -l ~/.bash_history"
echo "- grep -i password ~/.bash_history (should return nothing)"
echo "- grep 'ls -la' ~/.bash_history (should still exist)"
